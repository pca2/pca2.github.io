...
Closure in JavaScript has developed a bit of a reputation over the years, but it's actually nothing to be intimated by. Although it can be a bit tough to wrap your brain around initially, once you see in in action you'll be surprised at how simple it really can be. 
...
This string is defined inside funcOne
```
Here's another play-by of what's going on this time:

1. The `myFunc` variable is defined as the output of the `funcOne` function.
2. The `funcOne` function defines the `myString` variable as a string.
3. The `writeString` function is defined. All it does is log `myString`.
4. The `funcOne` function returns `writeString`, without calling it. 
5. The `myFunc` function is now called, which calls the `writeString` function.
6. The `writeString` function logs `myString`, which was defined way back in step 2 as part of `funcOne`.

Okay, while obviously there's a lot more going on here, from a compositional standpoint, it's very similiar code to the previous example. The key difference is that now instead of `funcOne` defining and then running `writeString`, it now defines it and _returns_ the function itself.
...
>A closure is the combination of a function and the lexical environment within which that functionwas declared. This environment consists of any local variables that were in- scope at the time that the closure was created. 
